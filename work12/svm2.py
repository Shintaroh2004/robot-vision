...
def load_images_from_folder(folder):
    ...
    for filename in os.listdir(folder):
        ...
        if img is not None:
            img = cv2.resize(img, (224, 224))  # VGG16は224x224の入力を要求する
            imgs.append(img)
...
def extract_features_vgg(images):
    features = []
    for img in images:
        img = cv2.resize(img, (224, 224))  # VGG16の入力サイズにリサイズ
        img = preprocess_input(img)  # 前処理（RGB平均の引き算など）
        img = np.expand_dims(img, axis=0)  # バッチ次元を追加
        feature = vgg_model.predict(img)  # VGG16で特徴量を抽出
        features.append(feature.flatten())  # フラット化してリストに追加
    return np.array(features)

# 特徴量の抽出（VGG16による）
features_vgg = extract_features_vgg(imgs)

# ラベルのエンコーディング
le = LabelEncoder()
lbls_encoded = le.fit_transform(lbls)

# データの分割
X_train, X_test, y_train, y_test, imgs_train, imgs_test = train_test_split(
    features_vgg, lbls_encoded, imgs, test_size=0.3, random_state=42
)
